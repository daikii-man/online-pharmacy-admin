...
export default function EmployyesPage() {
    ...
    const filteredEmployees = employees?.filter((el: any) => el?.name.toLocaleLowerCase().includes(search.toLocaleLowerCase()))

    const rowsPerPage = 10;

    const pages = filteredEmployees ? Math.ceil(filteredEmployees?.length / rowsPerPage) : 1;
            ...
            <div className="flex items-center justify-between mx-auto mt-24 mb-4">
                ...
                <div className="relative mt-2 rounded-md">
                    ...
                    <input
                        ...
                        value={search}
                        onChange={onChange}
                        placeholder={t('Emloyees.search.placeholder')}
                        className="block w-full bg-transparent rounded-md border-0 py-2.5 pl-12 pr-20 ring-1 ring-inset ring-gray-700 focus:ring-1 focus:ring-gray-600"
                    />
                </div>
            </div>
            <Table
                ...
                <TableHeader>
                    ...
                    <TableColumn className="px-12 w-80">{t('Emloyees.table.fullname')}</TableColumn>
                    <TableColumn className="w-56 px-5">{t('Emloyees.table.role')}</TableColumn>
                        ...
                        <TableRow key={employee.id} className="border-b">
                            ...
                            <TableCell className="w-28">
                                <div className="flex items-center gap-x-8">
                                    <span className="text-lg cursor-pointer text-danger active:opacity-50" onClick={() => editModalHandler(employee?.id)}>
                                        <svg
                                            aria-hidden="true"
                                            fill="none"
                                            focusable="false"
                                    ...
                                    <span className="text-lg cursor-pointer text-danger active:opacity-50" onClick={() => deleteModalHandler(employee?.id)}>
                                        <svg
                                            aria-hidden="true"
                                            fill="none"
                                            focusable="false"
